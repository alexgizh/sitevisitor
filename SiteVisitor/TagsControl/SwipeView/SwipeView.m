...
- (void)setUp
{
    ...
    _scrollView.decelerationRate = (double)_decelerationRate;
...
{
    if (fabs(_autoscroll - autoscroll) > 0.0001)
    {
...
- (void)updateScrollOffset
{
    if (_wrapEnabled)
    {
        CGFloat itemsWide = (_numberOfItems == 1)? 1.0: 3.0;
        if (_vertical)
        {
            ...
            if (_scrollView.contentOffset.y < scrollHeight)
            {
                ...
                [self setContentOffsetWithoutEvent:CGPointMake(0.0, _scrollView.contentOffset.y + scrollHeight)];
            }
            else if (_scrollView.contentOffset.y >= scrollHeight * 2.0)
            {
                _previousContentOffset.y -= scrollHeight;
                [self setContentOffsetWithoutEvent:CGPointMake(0.0, _scrollView.contentOffset.y - scrollHeight)];
            }
        ...
        else
        {
            ...
            if (_scrollView.contentOffset.x < scrollWidth)
            {
                ...
                [self setContentOffsetWithoutEvent:CGPointMake(_scrollView.contentOffset.x + scrollWidth, 0.0)];
            }
            else if (_scrollView.contentOffset.x >= scrollWidth * 2.0)
            {
                _previousContentOffset.x -= scrollWidth;
                [self setContentOffsetWithoutEvent:CGPointMake(_scrollView.contentOffset.x - scrollWidth, 0.0)];
            }
    ...
    if (_vertical && fabs(_scrollView.contentOffset.x) > 0.0001)
    {
        [self setContentOffsetWithoutEvent:CGPointMake(0.0, _scrollView.contentOffset.y)];
    }
    else if (!_vertical && fabs(_scrollView.contentOffset.y) > 0.0001)
    {
        [self setContentOffsetWithoutEvent:CGPointMake(_scrollView.contentOffset.x, 0.0)];
    }
}
...
- (void)updateScrollViewDimensions
{
    ...
    switch (_alignment)
    {
        case SwipeViewAlignmentCenter:
        {
            if (_vertical)
            {
                frame = CGRectMake(0.0, (self.bounds.size.height - _itemSize.height * _itemsPerPage)/2.0,
                                   self.bounds.size.width, _itemSize.height * _itemsPerPage);
                contentSize.height = _itemSize.height * _numberOfItems;
            }
            else
            {
                frame = CGRectMake((self.bounds.size.width - _itemSize.width * _itemsPerPage)/2.0,
                                   0.0, _itemSize.width * _itemsPerPage, self.bounds.size.height);
        ...
        case SwipeViewAlignmentEdge:
        {
            if (_vertical)
            {
                frame = CGRectMake(0.0, 0.0, self.bounds.size.width, _itemSize.height * _itemsPerPage);
                contentSize.height = _itemSize.height * _numberOfItems - (self.bounds.size.height - frame.size.height);
            }
            else
            {
                frame = CGRectMake(0.0, 0.0, _itemSize.width * _itemsPerPage, self.bounds.size.height);
    ...
    if (_wrapEnabled)
    {
        CGFloat itemsWide = (_numberOfItems == 1)? 1.0: _numberOfItems * 3.0;
        if (_vertical)
        {
    ...
    else if (_pagingEnabled && !_truncateFinalPage)
    {
        if (_vertical)
        {
            contentSize.height = (double)ceilf(contentSize.height / frame.size.height) * frame.size.height;
        }
        else
        {
            contentSize.width = (double)ceilf(contentSize.width / frame.size.width) * frame.size.width;
        }
    }
...
- (void)setFrameForView:(UIView *)view atIndex:(NSInteger)index
{
    if (self.window)
    {
        ...
        if (_vertical)
        {
            center.y = ([self offsetForItemAtIndex:index] + 0.5) * _itemSize.height + _scrollView.contentOffset.y;
        }
        else
        {
            center.x = ([self offsetForItemAtIndex:index] + 0.5) * _itemSize.width + _scrollView.contentOffset.x;
        }
        ...
        if (_vertical)
        {
            view.center = CGPointMake(_scrollView.frame.size.width/2.0, center.y);
        }
        else
        {
            view.center = CGPointMake(center.x, _scrollView.frame.size.height/2.0);
        }
        
        view.bounds = CGRectMake(0.0, 0.0, _itemSize.width, _itemSize.height);
...
- (void)didScroll
{
    ...
    if (!_defersItemViewLoading || fabs([self minScrollDistanceFromOffset:_lastUpdateOffset toOffset:_scrollOffset]) >= 1.0)
    {
        //update item index
...
- (void)step
{
    ...
    if (_scrolling)
    {
        NSTimeInterval time = fmin(1.0, (currentTime - _startTime) / _scrollDuration);
        delta = [self easeInOut:time];
        _scrollOffset = [self clampedOffset:_startOffset + (_endOffset - _startOffset) * delta];
        if (_vertical)
        {
            [self setContentOffsetWithoutEvent:CGPointMake(0.0, _scrollOffset * _itemSize.height)];
        }
        else
        {
            [self setContentOffsetWithoutEvent:CGPointMake(_scrollOffset * _itemSize.width, 0.0)];
        }
        [self didScroll];
        if (time == 1.0)
        {
...
- (CGFloat)clampedOffset:(CGFloat)offset
{
    ...
    if (_wrapEnabled)
    {
        returnValue = _numberOfItems? (offset - floor(offset / (CGFloat)_numberOfItems) * _numberOfItems): 0.0;
    }
    else
    {
        returnValue = fmin(fmax(0.0, offset), fmax(0.0, (CGFloat)_numberOfItems - 1.0));
    }
    return returnValue;
}
...
- (CGFloat)minScrollDistanceFromOffset:(CGFloat)fromOffset toOffset:(CGFloat)toOffset
{
    ...
    if (_wrapEnabled)
    {
        CGFloat wrappedDistance = fmin(toOffset, fromOffset) + _numberOfItems - fmax(toOffset, fromOffset);
...
{
    if (fabs(_scrollOffset - scrollOffset) > 0.0001)
    {
        _scrollOffset = scrollOffset;
        _lastUpdateOffset = _scrollOffset - 1.0; //force refresh
        _scrolling = NO; //stop scrolling
        [self updateItemSizeAndCount];
        [self updateScrollViewDimensions];
        [self updateLayout];
        CGPoint contentOffset = _vertical? CGPointMake(0.0, [self clampedOffset:scrollOffset] * _itemSize.height): CGPointMake([self clampedOffset:scrollOffset] * _itemSize.width, 0.0);
...
- (void)scrollViewWillBeginDragging:(__unused UIScrollView *)scrollView
{
    ...
    //force refresh
    _lastUpdateOffset = self.scrollOffset - 1.0;
    [self didScroll];
}
...
- (void)scrollViewDidEndDecelerating:(__unused UIScrollView *)scrollView
{
    ...
    CGFloat integerOffset = round(_scrollOffset);
    if (fabs(_scrollOffset - integerOffset) < 0.01)
    {
